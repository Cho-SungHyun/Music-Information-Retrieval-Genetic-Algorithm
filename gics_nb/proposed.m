function [opt_vec, ret_eval] = proposed( IDATA, IANSWER, IPOOL_SIZE,IFFCS, ENT_MAP)
global data
global answer
global pool
global col
...
pre = mcbayes_matlab( data(train,chr==1), answer(train,:), data(test,chr==1));

inter = answer(test,:) == pre;
lacc = sum(inter,1) / size(pre,1);

val = bayes_matlab(data(train,chr==1), answer(train,:), data(test,chr==1), answer(test,:));
end

%% NB for each class
function pre = mcbayes_matlab( train_x, train_y, test_x )

cNum = size(train_y,2);
pre = zeros( size(test_x,1), cNum );

for k=1:cNum
    model = fitcnb( train_x, train_y(:,k),  'DistributionNames', 'mvmn' );
